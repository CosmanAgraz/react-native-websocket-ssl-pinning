...
public class Socket {
  ...
  public boolean send(@NonNull String event, @NonNull String data){
    if (mRealWebSocket != null && mState == SocketState.OPEN) {
      try {
        return mRealWebSocket.send(data);
      } catch (Exception e) {
        Log.e(TAG, "Unable to send this data: " + data);
      }
    }
    return false;
  }
  ...
  private void reconnect() {
    if (mState != SocketState.CONNECT_ERROR || isForceTermination)
      return;
  ...
  private WebSocketListener webSocketListener = new WebSocketListener() {
    ...
    public void onOpen(WebSocket webSocket, Response response) {
      Log.i(TAG, "Socket has been opened successfully.");
    ...
    public void onMessage(WebSocket webSocket, String text) {
      ...
      try {
        ...
        postEvent(new SocketEvents.ResponseMessageEvent(event, text));

        // call event listener
        postEvent(new SocketEvents.BaseMessageEvent(event));
      } catch (Exception e) {
        Log.i(TAG,"Unknown error occurred while parsing message data.");
      }
    }
    ...
    public void onClosing(WebSocket webSocket, int code, String reason) {
      Log.i(TAG, "Close request from server with reason: " + reason);
    ...
    public void onFailure(WebSocket webSocket, Throwable t, Response response) {
      if (!isForceTermination) {
        ...
        Log.i(TAG, "Socket connection fail, try to reconnect. (" + reconnectionAttempts + ")");
